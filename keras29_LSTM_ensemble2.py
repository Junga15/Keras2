#LSTM 앙상블 1번 복붙 후 
#2개의 모델을하나는 LSTM 하나는 Dense로 앙상블 구현
#29_1번 성능비교
#29_2번이 성능이 더 좋아야함 => 왜?
#모델 구현시 무엇을 LSTM할지 생각
...
lstm1=LSTM(10,activation='relu')(input1) #이부분이 덴스 아닌 LSTM
dense1=Dense(10, activation='relu')(lstm1)
#output1=Dense(5)(dense1) =>어짜피 변수표기는 내가 하는 것이므로 마지막에 output1은 내 선택사항

#모델2_Dense모델로 구성
input2=Input(shape=(3,))  #덴스모델이므로 (13,3)을 인풋쉐이프로 넣어줄때 맨앞 날리고(3,) 
...
'''
x1_predict=x1_predict.reshape(1,3,1) #=>(1,3,1)로 리쉡 해준다는 것
x2_predict=x2_predict.reshape(1,x2.shape[1],1) #=>두번째모델은 덴스모델이므로 리쉡안함
